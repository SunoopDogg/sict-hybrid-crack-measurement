...
def get_weight(recn, branch):
    ...
    for i in range(recn.shape[0]*recn.shape[1]):
        ...
        w += (reconstruction[i] >
              0) ^ ((reconstruction[i] - branch_image[i]) > 0)
...
def _remove_mid_node(G):
    ...
    while True:
        ...
        dist = np.array(
            list(map(np.linalg.norm, [s1-s2, e1-e2, s1-e2, s2-e1])))
        if dist.argmin() == 0:
            line = np.concatenate(
                [edge1['pts'][::-1], [G.nodes[i]['o'].astype(np.int32)], edge2['pts']], axis=0)
        elif dist.argmin() == 1:
            line = np.concatenate(
                [edge1['pts'], [G.nodes[i]['o'].astype(np.int32)], edge2['pts'][::-1]], axis=0)
        elif dist.argmin() == 2:
            line = np.concatenate(
                [edge2['pts'], [G.nodes[i]['o'].astype(np.int32)], edge1['pts']], axis=0)
        elif dist.argmin() == 3:
            line = np.concatenate(
                [edge1['pts'], [G.nodes[i]['o'].astype(np.int32)], edge2['pts']], axis=0)

        G.add_edge(connected_nodes[0], connected_nodes[1],
                   weight=edge1['weight']+edge2['weight'], pts=line)
        G.remove_node(i)
    return G
