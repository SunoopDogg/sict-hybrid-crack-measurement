...
def get_pca_vector(skeleton_region, verbose=False):
    ...
    angle = math.atan2(eigenvectors[0, 0], eigenvectors[0, 1]) * 180 / math.pi
    if verbose:
...
def get_constant_region_pca(coord, skeleton, verbose=False):
    ...
    skeleton_region = skeleton[r-height:r+height+1,
                               c-width:c+width+1]  # gets the skeleton region
...
def get_adaptive_region_pca(coord, skeleton, diff=0.01, verbose=False):
    ...
    while True:
        ...
        v = get_pca_vector(skeleton_region, verbose)
        if len(v) > 0:
            angle_2 = math.atan2(v[0], v[1]) * 180 / \
                math.pi  # orientation in degrees
        else:
            return []
...
def get_width(pois, dt, bin_img, S, B, b, verbose=False, visualize=False):
    ...
    for poi in pois:
        ...
        # Apply threshold to search only local vicinity
        B_prime_dist_mask = np.linalg.norm(B_prime, axis=1) < radius
        ...
        if verbose:
            ...
            print(
                f"Projection coefffictient argmax : {np.argmax(projection_coeff)}")
            print(
                f"Projection coefffictient argmin : {np.argmin(projection_coeff)}")
        ...
        if visualize:
            ...
            # points used in the library are inverted
            end_pt = (pt2[1], pt2[0])
