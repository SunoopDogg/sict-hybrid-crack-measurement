...
def remove_intersection(binary_image, skl, boundary_coords, graph, verbose=False):
    ...
    points = [nodes[index[i]]['o'].squeeze()
              for i in range(len(index)) if graph.degree(index[i]) > 2]
    ...
    if len(points) > 0:
        for i, point in enumerate(points):
            ...
            while True:
                ...
                intersections = list(circle_points & set(
                    boundary_coords))  # Gets the intersection
                if len(intersections) >= 2*degrees[i]:
                    ...
                    binary_img[r-radius:r+radius+1, c-radius:c +
                               radius+1] -= morphology.disk(radius)
                    skeleton_img[r-radius:r+radius+1, c-radius:c +
                                 radius+1] -= morphology.disk(radius) * 1
                    break
                else:
                    radius += 1
            if verbose:
